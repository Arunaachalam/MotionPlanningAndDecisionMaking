...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      auto circle_center_x = cur_x + CIRCLE_OFFSETS[c] * std::cos(cur_yaw); //0;  // <- Update 
      auto circle_center_y = cur_y + CIRCLE_OFFSETS[c] * std::sin(cur_yaw); //0;  // <- Update 
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double dist = std::sqrt(std::pow(circle_center_x - actor_center_x, 2) + std::pow(circle_center_y - actor_center_y, 2));//0;  // <- Update
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  auto delta_x = main_goal.location.x - spiral[n - 1].x; //0;  // <- Update
  auto delta_y = main_goal.location.y - spiral[n - 1].y; //0;  // <- Update
  auto delta_z = main_goal.location.z - spiral[n - 1].z; //0;  // <- Update
