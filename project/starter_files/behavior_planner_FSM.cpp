...
double BehaviorPlannerFSM::get_look_ahead_distance(const State& ego_state) {
  ...
  auto look_ahead_distance = (velocity_mag * velocity_mag) / (2.0 * accel_mag) ;//1.0;  // <- Fix This
...
State BehaviorPlannerFSM::state_transition(const State& ego_state, State goal,
  ...
  if (_active_maneuver == FOLLOW_LANE) {
    ...
    if (is_goal_in_junction) {
      ...
      goal.location.x += _stop_line_buffer * std::cos(ang);//1.0;  // <- Fix This
      goal.location.y += _stop_line_buffer * std::sin(ang);//1.0;  // <- Fix This

      // LOG(INFO) << "BP- new STOP goal at: " << goal.location.x << ", "
      //          << goal.location.y;

      // TODO-goal speed at stopping point: What should be the goal speed??
      goal.velocity.x = 0.0; //1.0;  // <- Fix This
      goal.velocity.y = 0.0; //1.0;  // <- Fix This
      goal.velocity.z = 0.0; //1.0;  // <- Fix This
      ...
      goal.velocity.x = _speed_limit * std::cos(goal.rotation.yaw); // 1.0;  // <- Fix This
      goal.velocity.y = _speed_limit * std::sin(goal.rotation.yaw); // 1.0;  // <- Fix This
      goal.velocity.z = 0;
    }
    ...
    if (distance_to_stop_sign <+ P_STOP_THRESHOLD_DISTANCE){//utils::magnitude(ego_state.velocity) <=
